...
routes the approval decision:
  ...
  manual        -> POST to VI approval API, block until human decides
  timeout_allow -> POST to VI approval API, block until decided or timeout, then auto-approve

Exit codes:
  0 -> Claude Code proceeds with the tool call
  2 -> Claude Code blocks the tool; stdout JSON {"reason": "..."} is shown to the AI

Environment variables expected (set by vi-agent before launching Claude Code):
  VI_AGENT_STATE_FILE  - path to the vi-agent state JSON
  VI_SERVER            - VI dashboard URL
  VI_AGENT_ID          - agent ID
  VI_REMOTE_JOB_ID     - job ID (optional, for linking approval to the job)
"""
...
def _create_approval_request(
    ...
    """Create a VI approval request and return requestId, or None on failure."""
    ...
    try:
        ...
        print(f"[vi-approve] Failed to create approval request: {exc}", file=sys.stderr)
        return None
...
def main() -> None:
    ...
    state_file = os.environ.get("VI_AGENT_STATE_FILE", "")
    server = os.environ.get("VI_SERVER", "").rstrip("/")
    agent_id = os.environ.get("VI_AGENT_ID", "")
    job_id = os.environ.get("VI_REMOTE_JOB_ID", "")
    ...
    # manual / timeout_allow: route through VI approval API
    if not server or not agent_id:
        ...
        _deny(
            f"VI approval not configured (missing VI_SERVER or VI_AGENT_ID). "
            f"{tool_name} is blocked. Check that vi-agent set the required env vars."
        )
    ...
    if not request_id:
        ...
        _deny(f"VI approval server unreachable. {tool_name} blocked. Check the VI dashboard.")
    ...
    if decision == "approved":
        ...
        if permission_mode == "timeout_allow":
            ...
            _deny(f"VI approval timed out for {tool_name}. No decision received.")
