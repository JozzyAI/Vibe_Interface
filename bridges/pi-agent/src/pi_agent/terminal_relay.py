"""
PI Terminal Relay Client — pi-agent side.

Connects to the PI server's /pi-agent-relay WebSocket endpoint and serves
...
PI_SESSION_PREFIX = "pi_remote_"  # tmux session name prefix used by launch_remote_job


def discover_pi_tmux_sessions(tmux_path: str | None = None, prefix: str = PI_SESSION_PREFIX) -> list[str]:
    """
    List local tmux sessions whose names start with prefix (default: "pi_remote_").
...
def derive_terminal_relay_url(server: str, explicit_url: str | None = None) -> str | None:
    """
    Build the WebSocket URL for the /pi-agent-relay endpoint.

    Priority:
      1. explicit_url (PI_TERMINAL_RELAY_URL env var) if set — used as-is
      2. Derived from PI_SERVER:
           http://host:port  → ws://host:<direct_port>/pi-agent-relay   (local dev)
           https://host      → wss://host/pi-agent-relay                 (production via TLS proxy)

    In local development, PI_SERVER is typically http://host:3000.
    The direct-terminal-ws server listens on port 14801 (or DIRECT_TERMINAL_PORT).
    In production behind an nginx/Caddy proxy, /pi-agent-relay is served at the
    same host as PI_SERVER over TLS, so port 14801 is not exposed directly.
    """
    if explicit_url:
        url = explicit_url.rstrip("/")
        if not url.endswith("/pi-agent-relay"):
            url += "/pi-agent-relay"
        return url
    ...
    if s.startswith("https://") or s.startswith("wss://"):
        # Production: proxy handles TLS, /pi-agent-relay is at the same host
        host = s.split("://", 1)[1].split("/")[0]
        return f"wss://{host}/pi-agent-relay"
    else:
        # Local dev: direct-terminal-ws on a separate port
        host = s.split("://", 1)[-1].split("/")[0].split(":")[0]
        return f"ws://{host}:{direct_port}/pi-agent-relay"
...
class TerminalRelayClient:
    """
    Background thread that maintains the pi-agent → PI /pi-agent-relay WebSocket.

    Usage:
        client = TerminalRelayClient(relay_url, agent_id, token)
        client.start()              # starts background thread
        client.announce("pi_remote_job_xxx")  # call after a tmux session is created
    ...
    def __init__(self, relay_url: str, agent_id: str, token: str):
        if _websocket_lib is None:
            raise RuntimeError(
                ...
                "Reinstall pi-agent: pip install pi-agent"
            )
        self._url = relay_url
    ...
    def _connect_and_serve(self) -> None:
        ...
        # Merge any local pi_remote_* tmux sessions that exist on this machine
