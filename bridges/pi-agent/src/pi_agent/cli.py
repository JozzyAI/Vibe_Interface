...
def handoff_enabled() -> bool:
    return os.environ.get("PI_HANDOFF_ENABLED", "0").strip().lower() in {"1", "true", "yes", "on"}
...
def handoff_file_is_pi_managed(path: Path) -> bool:
    try:
        return path.read_text(encoding="utf-8", errors="replace").lstrip().startswith("<!-- PI_MANAGED_HANDOFF -->")
    except OSError:
        return False
...
def refresh_managed_handoff_files(
    ...
    *,
    ...
    notes = (
        ...
        "- If a human or agent removes the PI_MANAGED_HANDOFF marker, PI will stop overwriting this file.\n"
    )
    write_if_changed(progress_path, progress)
    write_if_changed(todo_path, todo)
    write_if_changed(notes_path, notes)
...
def should_allocate_pty(command: list[str]) -> bool:
    joined = " ".join(command)
    return " pi_agent.cli codex" in joined or " pi_agent.cli claude" in joined
...
def default_state_file_from_values(project: str, tool: str, display_name: str) -> str:
    base_dir = Path.home() / ".config" / "pi-agent"
...
def default_state_file(args: argparse.Namespace) -> str:
    return default_state_file_from_values(
        env_or(args, "project", "PI_AGENT_PROJECT", "default-project") or "default-project",
        env_or(args, "tool", "PI_AGENT_TOOL", "agent") or "agent",
        env_or(args, "display_name", "PI_AGENT_DISPLAY_NAME", "remote") or "remote",
    )


def resolve_state_file(args: argparse.Namespace) -> Path:
    explicit = env_or(args, "state_file", "PI_AGENT_STATE_FILE")
...
def normalize_bridge_command(command: list[str]) -> list[str]:
    if command and command[0] == "pi-agent":
        return [sys.executable, "-m", "pi_agent.cli", *command[1:]]
    return command
...
def serialize_bridge_context(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        or config_value(args, "display_name", "PI_AGENT_DISPLAY_NAME", state_path=state_path, paired_key="displayName"),
        "project": agent.get("projectLabel")
        or config_value(args, "project", "PI_AGENT_PROJECT", state_path=state_path, paired_key="projectLabel"),
        "tool": agent.get("toolType")
        or config_value(args, "tool", "PI_AGENT_TOOL", state_path=state_path, paired_key="toolType"),
        "host": agent.get("hostLabel")
        or config_value(args, "host", "PI_AGENT_HOST", state_path=state_path, paired_key="hostLabel"),
        "repoRoot": agent.get("repoRoot")
        or config_value(args, "repo_root", "PI_AGENT_REPO_ROOT", state_path=state_path, paired_key="repoRoot"),
        "worktree": agent.get("worktree")
        or config_value(args, "worktree", "PI_AGENT_WORKTREE", state_path=state_path, paired_key="worktree"),
        "status": agent.get("status")
        or config_value(args, "status", "PI_AGENT_STATUS", state_path=state_path, paired_key="status"),
...
def resolve_agent_id(args: argparse.Namespace) -> str:
    direct = env_or(args, "agent_id", "PI_AGENT_ID")
    if direct:
        return direct
...
class RelayClient:
    def __init__(self, relay_url: str, relay_token: str, peer_id: str, label: str | None = None):
        if websocket is None:
            raise RuntimeError(
                "Relay support requires websocket-client. Reinstall pi-agent so the relay dependency is available."
            )
...
def register_agent(args: argparse.Namespace) -> dict[str, Any]:
    ...
    state = load_state_file(state_path)
    tool_type = require(
        config_value(args, "tool", "PI_AGENT_TOOL", state_path=state_path, paired_key="toolType"),
        "--tool",
    )
    payload = {
        "agentId": env_or(args, "agent_id", "PI_AGENT_ID"),
        "displayName": require(
            config_value(args, "display_name", "PI_AGENT_DISPLAY_NAME", state_path=state_path, paired_key="displayName"),
            "--display-name",
        ),
        "projectLabel": require(
            config_value(args, "project", "PI_AGENT_PROJECT", state_path=state_path, paired_key="projectLabel"),
            "--project",
        ),
        "toolType": tool_type,
        ...
        "branch": config_value(args, "branch", "PI_AGENT_BRANCH", state_path=state_path, paired_key="branch"),
    ...
    server = normalize_server(
        require(config_value(args, "server", "PI_SERVER", state_path=state_path, paired_key="server"), "--server")
    )
...
def heartbeat_agent(args: argparse.Namespace) -> dict[str, Any]:
    state_path = resolve_state_file(args)
    state = load_state_file(state_path)
    tool_type = config_value(args, "tool", "PI_AGENT_TOOL", state_path=state_path, paired_key="toolType") or ""
    payload = {
        "agentId": resolve_agent_id(args),
        "status": config_value(args, "status", "PI_AGENT_STATUS", state_path=state_path, paired_key="status"),
        "branch": config_value(args, "branch", "PI_AGENT_BRANCH", state_path=state_path, paired_key="branch"),
        "worktree": config_value(
            args,
            "worktree",
...
def poll_agent(args: argparse.Namespace) -> dict[str, Any]:
    ...
    server = normalize_server(
        require(
            config_value(args, "server", "PI_SERVER", state_path=resolve_state_file(args), paired_key="server"),
            "--server",
        )
    )
...
def handoff_update(args: argparse.Namespace) -> dict[str, Any]:
    job_id = require(
        config_value(args, "job_id", "PI_REMOTE_JOB_ID", state_path=resolve_state_file(args), paired_key=None),
        "--job-id",
    )
    if not handoff_enabled():
        return report_job(args, job_id, "running")
    artifact_dir_value = config_value(args, "session_dir", "PI_SESSION_DIR", state_path=resolve_state_file(args), paired_key=None)
...
def request_approval(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload = {
        "agentId": agent_id,
        "parentJobId": getattr(args, "parent_job_id", None) or env_or(args, "parent_job_id", "PI_REMOTE_JOB_ID"),
...
def pair_bridge(args: argparse.Namespace) -> int:
    server = normalize_server(require(env_or(args, "server", "PI_SERVER"), "--server"))
    ...
    explicit_state = env_or(args, "state_file", "PI_AGENT_STATE_FILE")
    if explicit_state:
    ...
    result: dict[str, Any] = {
        ...
        "projectLabel": config.get("projectLabel"),
        "toolType": config.get("toolType"),
        "nextCommand": f"pi-agent start-daemon --state-file {shlex.quote(str(state_path))}",
    }
...
def run_wrapped(args: argparse.Namespace) -> int:
    ...
    heartbeat_ms = int(env_or(args, "heartbeat_ms", "PI_AGENT_HEARTBEAT_MS", "5000") or "5000")
...
def approve_command(args: argparse.Namespace) -> int:
    ...
    decision = request_approval(
        argparse.Namespace(
            server=env_or(args, "server", "PI_SERVER"),
            agent_id=env_or(args, "agent_id", "PI_AGENT_ID"),
            state_file=env_or(args, "state_file", "PI_AGENT_STATE_FILE"),
            title=title,
            message=message,
    ...
    return subprocess.call(
        command,
        cwd=env_or(args, "cwd", "PI_AGENT_WORKTREE", os.getcwd()),
        env=os.environ.copy(),
    )
...
def run_daemon(args: argparse.Namespace) -> int:
    ...
    handoff_idle_seconds = max(60, int(os.environ.get("PI_HANDOFF_IDLE_SECONDS", "600") or "600"))
    launched_jobs: dict[str, dict[str, Any]] = {}
    consecutive_failures = 0
    relay_url = config_value(args, "relay_url", "PI_RELAY_URL", state_path=state_path, paired_key="relay.url")
    ...
    def request_pi_utility_session_from_runtime(job_id: str, screen: str) -> None:
        ...
        try:
            ...
            if relay_base and headers:
                ...
                server = normalize_server(
                    require(
                        config_value(args, "server", "PI_SERVER", state_path=state_path, paired_key="server"),
                        "--server",
                    )
                )
    ...
    def launch_remote_job(job: dict[str, Any]) -> None:
        ...
        if handoff_enabled():
            child_env.update(
                {
                    "PI_HOOK_HANDOFF_UPDATE": "pi-agent handoff-update",
                    "PI_HANDOFF_STATE_FILE": str(artifact_dir / "handoff-state.json"),
                    "PI_PROGRESS_FILE": str(artifact_dir / "PROGRESS.md"),
                    "PI_TODO_FILE": str(artifact_dir / "TODO.md"),
                    "PI_NOTES_FILE": str(artifact_dir / "NOTES.md"),
                }
            )
        ...
        if tmux_available() and (force_interactive or is_interactive_remote_command(command)):
            ...
            # PI_INITIAL_GOAL: clean user prompt injected as /plan for Claude (no hook instructions).
            ...
            try:
                if relay_url and relay_token and relay_client is None:
                    relay_client = RelayClient(
                        ...
                        peer_id=agent_id,
                        label=agent.get("displayName") or env_or(args, "display_name", "PI_AGENT_DISPLAY_NAME"),
                    )
            ...
            if time.time() >= next_sync_at:
                try:
                    heartbeat_args = argparse.Namespace(
                        ...
                        relay_token=relay_token,
                        agent_id=agent_id,
                        status=config_value(args, "status", "PI_AGENT_STATUS", state_path=state_path, paired_key="status", default="running"),
                        branch=config_value(args, "branch", "PI_AGENT_BRANCH", state_path=state_path, paired_key="branch"),
                        worktree=config_value(args, "worktree", "PI_AGENT_WORKTREE", state_path=state_path, paired_key="worktree", default=os.getcwd()),
...
def start_daemon(args: argparse.Namespace) -> int:
    ...
    resolved_values = {
        "PI_SERVER": config_value(args, "server", "PI_SERVER", state_path=state_path, paired_key="server"),
        "PI_AGENT_ID": env_or(args, "agent_id", "PI_AGENT_ID"),
        "PI_AGENT_DISPLAY_NAME": config_value(args, "display_name", "PI_AGENT_DISPLAY_NAME", state_path=state_path, paired_key="displayName"),
        "PI_AGENT_PROJECT": config_value(args, "project", "PI_AGENT_PROJECT", state_path=state_path, paired_key="projectLabel"),
        "PI_AGENT_TOOL": config_value(args, "tool", "PI_AGENT_TOOL", state_path=state_path, paired_key="toolType"),
        "PI_AGENT_HOST": config_value(args, "host", "PI_AGENT_HOST", state_path=state_path, paired_key="hostLabel"),
        "PI_AGENT_REPO_ROOT": config_value(args, "repo_root", "PI_AGENT_REPO_ROOT", state_path=state_path, paired_key="repoRoot"),
        "PI_AGENT_BRANCH": config_value(args, "branch", "PI_AGENT_BRANCH", state_path=state_path, paired_key="branch"),
        "PI_AGENT_WORKTREE": config_value(args, "worktree", "PI_AGENT_WORKTREE", state_path=state_path, paired_key="worktree"),
        "PI_AGENT_STATUS": config_value(args, "status", "PI_AGENT_STATUS", state_path=state_path, paired_key="status"),
        "PI_AGENT_STATE_FILE": str(state_path),
        "PI_RELAY_URL": config_value(args, "relay_url", "PI_RELAY_URL", state_path=state_path, paired_key="relay.url"),
        "PI_RELAY_TOKEN": config_value(args, "relay_token", "PI_RELAY_TOKEN", state_path=state_path, paired_key="relay.token"),
    }
    for key, value in resolved_values.items():
        if value:
            env[key] = value

    if not load_paired_config(state_path) and resolved_values.get("PI_SERVER"):
        update_state_file(
            state_path,
            pairedConfig={
                "server": resolved_values.get("PI_SERVER"),
                "displayName": resolved_values.get("PI_AGENT_DISPLAY_NAME"),
                "projectLabel": resolved_values.get("PI_AGENT_PROJECT"),
                "toolType": resolved_values.get("PI_AGENT_TOOL"),
                "relay": {
                    "url": resolved_values.get("PI_RELAY_URL"),
                    "token": resolved_values.get("PI_RELAY_TOKEN"),
                },
            },
        )
    ...
    heartbeat_ms = env_or(args, "heartbeat_ms", "PI_AGENT_HEARTBEAT_MS")
    if heartbeat_ms:
    ...
    popen_kwargs: dict[str, Any] = {
        ...
        "stderr": subprocess.STDOUT,
        "env": env,
        "cwd": config_value(args, "worktree", "PI_AGENT_WORKTREE", state_path=state_path, paired_key="worktree", default=os.getcwd()),
        "close_fds": True,
    }
    if os.name == "nt":
        creationflags = 0
