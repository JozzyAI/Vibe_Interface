    ...
    startTransition(() => {
      void (async () => {
        try {
          ...
          const idea = await requestJson<VIIdeaBoardData>("/api/vi/ideas/board", {
            method: "POST",
            body: JSON.stringify({
              projectId,
              title,
          ...
          const result = await requestJson<{ job: RemoteAgentJob }>("/api/remote-agents/jobs", {
            ...
            body: JSON.stringify({
              ...
              env: {
                VI_WORKBENCH_TASK_TITLE: title,
                VI_WORKBENCH_IDEA_COUNT: String(idea.columns.flatMap((column) => column.ideas).length),
              },
            }),
          });
        ...
        try {
          ...
          const result = await requestJson<{ job: RemoteAgentJob }>("/api/remote-agents/jobs", {
            ...
            body: JSON.stringify({
              ...
              env: {
                VI_RESUME_SESSION_ID: selectedResumeSession.sessionId,
                VI_RESUME_SESSION_PATH: selectedResumeSession.path,
              },
            }),
          });
      ...
      <aside className="hidden w-[320px] shrink-0 flex-col border-r border-[#ececea] bg-white lg:flex">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="rounded-2xl border border-[#e8e8e5] bg-[#fbfbfa] p-4 text-[13px] leading-6 text-[#737882]">
            <p className="font-semibold text-[#30333a]">{projectName ?? "Vibe Interface"}</p>
