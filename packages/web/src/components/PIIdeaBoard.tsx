...
type ColumnsPayload = Record<PIIdeaStatus, string[]>;
...
const COLUMN_ORDER: PIIdeaStatus[] = ["idea_bank", "project_queue", "working", "done"];
...
function buildColumnsPayload(board: PIIdeaBoardData): ColumnsPayload {
  return {
...
function moveIdea(
  board: PIIdeaBoardData,
  ideaId: string,
  targetColumn: PIIdeaStatus,
  beforeIdeaId?: string,
): PIIdeaBoardData {
  ...
  let movingIdea: PIIdeaCard | null = null;
...
function findIdea(board: PIIdeaBoardData | undefined, ideaId: string | null): PIIdeaCard | null {
...
function joinWorkspace(root: string | undefined, idea: PIIdeaCard): string {
...
function buildIdeaPrompt(idea: PIIdeaCard, workspace: string): string {
  return [
    `[PI_IDEA_ID:${idea.id}]`,
...
export function PIIdeaBoard({
  ...
  const [board, setBoard] = useState<PIIdeaBoardData | undefined>(initialData);
  ...
  const boardUrl = projectId
    ? `/api/pi/ideas/board?project=${encodeURIComponent(projectId)}`
    : "/api/pi/ideas/board";

  const refresh = async () => {
    setBoard(await requestJson<PIIdeaBoardData>(boardUrl));
  };
  ...
  const persistColumns = (nextBoard: PIIdeaBoardData) =>
    startTransition(() => {
      void (async () => {
        try {
          setError(null);
          setBoard(nextBoard);
          const latest = await requestJson<PIIdeaBoardData>("/api/pi/ideas/board", {
            method: "PATCH",
            body: JSON.stringify({
                projectId,
    ...
    startTransition(() => {
      void (async () => {
        try {
          ...
          const nextBoard = await requestJson<PIIdeaBoardData>("/api/pi/ideas/board", {
  ...
  const openIdea = (idea: PIIdeaCard) => {
    if (mode !== "full") return;
  ...
  const ideasFor = (status: PIIdeaStatus): PIIdeaCard[] =>
    board?.columns.find((column) => column.id === status)?.ideas ?? [];

  const unqueueIdea = (idea: PIIdeaCard) => {
    if (!board) return;
    persistColumns(moveIdea(board, idea.id, "idea_bank"));
  };

  const sendIdeaToAgent = (idea: PIIdeaCard) =>
    startTransition(() => {
      void (async () => {
        try {
          setError(null);
...
function IdeaCardList(props: {
  ...
  onPrimaryAction: (idea: PIIdeaCard) => void;
  onOpenIdea: (idea: PIIdeaCard) => void;
...
function IdeaColumns(props: {
  ...
  onDragEnd: () => void;
  onDropIdea: (targetColumn: PIIdeaStatus, beforeIdeaId?: string) => void;
  onOpenIdea: (idea: PIIdeaCard) => void;
}) {
  if (!props.board) {
    return (
