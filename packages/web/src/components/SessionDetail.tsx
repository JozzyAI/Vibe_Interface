...
function getCiDotBg(pr: DashboardPR): string {
  ...
  if (pr.ciStatus === "passed") return "var(--color-accent-green)";
  if (pr.ciStatus === "failed") return "var(--color-accent-red)";
...
function getCiShortLabel(pr: DashboardPR): string {
  ...
  if (pr.ciStatus === "passed") return "CI passing";
  if (pr.ciStatus === "failed") return "CI failed";
  return "CI pending";
}
...
function SessionRuntimeStatus({ session }: { session: DashboardSession }) {
  const runtimeHandle = session.metadata["runtimeHandle"];
  const hostLabel = session.metadata["tmuxName"] ?? session.metadata["host"] ?? null;
  const worktree = session.metadata["worktree"] ?? null;

  if (!runtimeHandle && !hostLabel && !worktree) return null;

  return (
    <section className="mb-5 rounded-[28px] border border-[var(--color-accent-green)]/25 bg-[var(--color-accent-green-soft)] px-5 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="rounded-full border border-[var(--color-accent-green)]/35 px-3 py-1 text-[11px] font-semibold text-[var(--color-accent-green)]">
          {session.status}
        </span>
      </div>

      <div className="mt-3 grid gap-2 text-[12px] text-[var(--color-text-secondary)]">
        {hostLabel ? <p>Host: {hostLabel}</p> : null}
        {worktree ? <p>Worktree: {worktree}</p> : null}
        <p>Last activity: {formatTimeCompact(session.lastActivityAt)}</p>
      </div>
    </section>
  );
}

function SessionAutomationControls({
  session,
  disabled,
}: {
  session: DashboardSession;
  ...
  useEffect(() => {
    try {
      ...
      setRalphEnabled(Boolean(parsed.ralphEnabled));
      setAutoResumeUsageLimit(Boolean(parsed.autoResumeUsageLimit));
    } catch {
      // Ignore corrupt local state; the controls can be toggled again.
    }
  }, [storageKey]);

  const persist = useCallback(
    (next: { ralphEnabled: boolean; autoResumeUsageLimit: boolean }) => {
  ...
  const toggleRalph = useCallback(async () => {
    const next = !ralphEnabled;
    setRalphEnabled(next);
    persist({ ralphEnabled: next, autoResumeUsageLimit });
    setMessage(null);
    ...
    try {
      ...
      setMessage(error instanceof Error ? `Could not notify agent: ${error.message}` : "Could not notify agent.");
    } finally {
      setIsSaving(null);
    }
  }, [autoResumeUsageLimit, disabled, persist, ralphEnabled, sendControlMessage]);

  const toggleUsageLimitResume = useCallback(async () => {
    const next = !autoResumeUsageLimit;
    setAutoResumeUsageLimit(next);
    persist({ ralphEnabled, autoResumeUsageLimit: next });
    setMessage(null);
    if (!next || disabled) return;
    setIsSaving("usage");
    try {
  ...
  if (session.metadata["agent"] === "orchestrator") return null;

  return (
    <section className="mb-5 rounded-[28px] border border-[var(--color-border-default)] bg-[var(--color-bg-surface)] px-5 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-[18px] font-semibold text-[var(--color-text-primary)]">
            Keep this session running when it is safe
          </h2>
        </div>
        {disabled ? (
          <span className="rounded-full border border-[var(--color-border-default)] px-3 py-1 text-[11px] font-semibold text-[var(--color-text-tertiary)]">
            ended
          </span>
        ) : null}
      </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        >
          <span className="block text-[12px] font-semibold text-[var(--color-text-primary)]">
            {ralphEnabled ? "Ralph loop enabled" : "Enable Ralph loop"}
          </span>
          <span className="mt-1 block text-[12px] leading-5 text-[var(--color-text-secondary)]">
            Let this session keep choosing the next useful step until blocked or stopped.
          </span>
        </button>
        ...
        <button
          type="button"
          disabled={isSaving !== null}
          onClick={() => void toggleUsageLimitResume()}
          className={[
            "rounded-3xl border px-4 py-3 text-left transition disabled:cursor-not-allowed disabled:opacity-60",
      ...
      {message ? (
        <p className="mt-3 text-[12px] leading-5 text-[var(--color-text-secondary)]">{message}</p>
      ) : null}
    </section>
  );
}
...
export function SessionDetail({
  ...
  }, [dashboardHref, session.id]);

  const handleDelete = useCallback(async () => {
    const confirmed = window.confirm(
      "Delete this session permanently? This removes the session record and cannot be undone.",
    );
    if (!confirmed) return;

    try {
      const res = await fetch(`/api/sessions/${encodeURIComponent(session.id)}/delete`, { method: "POST" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      window.location.href = dashboardHref;
    } catch (err) {
      console.error("Failed to delete session permanently:", err);
    }
  }, [dashboardHref, session.id]);
  ...
  const showHeaderProjectLabel =
    headerProjectLabel.trim().toLowerCase() !== "project-interface";
                ...
                <main className="min-w-0">
                  {(!isOrchestrator || (isOrchestrator && orchestratorZones)) && (
                    <SessionTopStrip
                      ...
                      crumbHref={crumbHref}
                      crumbLabel={crumbLabel}
                      onArchive={isOrchestrator || terminalEnded ? undefined : handleArchive}
                      onDelete={isOrchestrator ? undefined : handleDelete}
                  ...
                  <SessionRuntimeStatus session={session} />

                  <SessionAutomationControls
                    session={session}
