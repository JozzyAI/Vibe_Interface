...
type WallItem =
  | {
      ...
      requests: PIApprovalInboxEntry[];
      session: PIApprovalFleetItem;
    }
  | {
      kind: "remote";
      key: string;
...
const MODE_META: Record<PIApprovalPermissionMode, { label: string; next: PIApprovalPermissionMode }> = {
...
function modeLabel(mode: PIApprovalPermissionMode): string {
...
export function PIApprovalInbox({ initialCards, initialRemoteOverview, hideSummary }: Props) {
  ...
  const refreshProject = async (projectId: string): Promise<PIApprovalHubData> =>
    requestJson<PIApprovalHubData>(`/api/pi/approval-hub?project=${encodeURIComponent(projectId)}`);
  ...
  const updateCard = (projectId: string, hub: PIApprovalHubData) => {
    setCards((current) =>
  ...
  const alwaysApprove = (projectId: string, request: PIApprovalInboxEntry) =>
    startTransition(() => {
      void (async () => {
        try {
          setError(null);
  ...
  const updateSessionPolicy = (
    projectId: string,
    sessionId: string,
    currentMode: PIApprovalPermissionMode,
    timeoutSeconds: number,
  ) =>
    startTransition(() => {
      void (async () => {
        try {
          setError(null);
          const nextHub = await requestJson<PIApprovalHubData>("/api/pi/approval-hub/policy", {
            method: "POST",
            body: JSON.stringify({
              projectId,
              sessionId,
  ...
  const updateRemotePolicy = (agent: RemoteAgentSummary, mode?: PIApprovalPermissionMode) =>
    startTransition(() => {
      void (async () => {
        try {
          setError(null);
      ...
      <SessionSection title="Needs attention" empty="No pending approvals or broken sessions right now.">
        {needsAttention.map((item) => (
          <SessionWallCard
            ...
            onApprove={(request) =>
              item.kind === "local"
                ? respond(item.projectId, request as PIApprovalInboxEntry, "approve")
                : respondRemote(request as RemoteApprovalRequest, "approve", item.agent)
            }
            onAlwaysApprove={(request) =>
              item.kind === "local"
                ? alwaysApprove(item.projectId, request as PIApprovalInboxEntry)
                : respondRemote(request as RemoteApprovalRequest, "approve", item.agent, true)
            }
            onReject={(request) =>
              item.kind === "local"
                ? respond(item.projectId, request as PIApprovalInboxEntry, "reject")
...
function SessionWallCard({
  ...
  onModeClick: () => void;
  onApprove?: (request: PIApprovalInboxEntry | RemoteApprovalRequest) => void;
  onAlwaysApprove?: (request: PIApprovalInboxEntry | RemoteApprovalRequest) => void;
  onReject?: (request: PIApprovalInboxEntry | RemoteApprovalRequest) => void;
  onArchive?: () => void;
}) {
...
function ApprovalMiniCard({
  ...
  request: PIApprovalInboxEntry | RemoteApprovalRequest;
  isPending: boolean;
  onApprove: () => void;
