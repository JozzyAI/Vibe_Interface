...
const VI_SERVER_ORIGIN = "http://192.168.1.83:3000";
...
function reconnectCommand(agent: RemoteAgentSummary, serverOrigin: string): string | null {
  ...
  return `vi-agent start-daemon --state-file ${agent.stateFile} --server ${serverOrigin}`;
}
...
export function VIAgentsEntry({ initialRemoteOverview, selectedAgentId, view = "detail" }: Props) {
  ...
  // Local mode: derive from VI_SERVER_ORIGIN (LAN IP).
...
function MachineRow({
  ...
  const [isPending, startTransition] = useTransition();
  // Session history is only populated for Codex sessions by vi-agent; use it directly.
        ...
        try {
          ...
          const result = await requestJson<{ job: RemoteAgentJob }>("/api/remote-agents/jobs", {
            ...
            body: JSON.stringify({
              ...
              env: {
                VI_RESUME_SESSION_ID: session.sessionId,
                VI_RESUME_SESSION_PATH: session.path,
              },
            }),
          });
