...
export async function GET(request: NextRequest) {
  ...
  if (oauthError) {
    if (state) {
      const consumed = await consumeVIGitHubOAuthState(config.configPath, state);
      if (consumed?.popup) {
  ...
  try {
    ...
    const currentStore = await listVIGitHubConnectors(config.configPath, project.path);
    const reusable =
