...
import { listVIGitHubConnectors, upsertVIGitHubConnector, type VIGitHubConnectorStore } from "@vi/core";
...
export async function GET(request: NextRequest) {
  ...
  const store = await listVIGitHubConnectors(config.configPath, config.projects[projectId].path);
...
export async function POST(request: NextRequest) {
  ...
  const currentStore = await listVIGitHubConnectors(config.configPath, config.projects[projectId].path);
  const connectorId =
  ...
  const store = await upsertVIGitHubConnector(config.configPath, config.projects[projectId].path, {
    id: connectorId,
