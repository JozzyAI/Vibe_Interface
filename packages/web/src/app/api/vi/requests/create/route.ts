...
import { upsertVIPendingQuestion } from "@vi/core";
import { getServices } from "@/lib/services";
import { getCorrelationId, jsonWithCorrelation } from "@/lib/observability";
import { appendVIApprovalAuditEvent, getVIApprovalPolicy } from "@/lib/vi-approval-hub";
...
export async function POST(request: NextRequest) {
  ...
  const policy = await getVIApprovalPolicy({
  ...
  const pending = await upsertVIPendingQuestion(config.configPath, project.path, sessionId, {
    id: requestId,
    sessionId,
    kind,
    title,
