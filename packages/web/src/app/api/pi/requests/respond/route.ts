...
import {
  respondToPIPendingQuestion,
  upsertPIPendingQuestion,
  type PIRequestKind,
} from "@pi/core";
import { getServices } from "@/lib/services";
import { getCorrelationId, jsonWithCorrelation } from "@/lib/observability";
import { appendPIApprovalAuditEvent } from "@/lib/pi-approval-hub";
...
export async function POST(request: NextRequest) {
  ...
  let updated = await respondToPIPendingQuestion(
    config.configPath,
    project.path,
    sessionId,
    requestId,
  ...
  if (!updated) {
    updated = await upsertPIPendingQuestion(config.configPath, project.path, sessionId, {
