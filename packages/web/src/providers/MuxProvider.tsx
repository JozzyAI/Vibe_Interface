...
function buildMuxWsUrl(runtimeConfig: { directTerminalPort?: string; proxyWsPath?: string }): string {
  ...
  // Runtime proxy path takes priority (set via TERMINAL_WS_PATH env var)
  ...
  if (loc.port === "" || loc.port === "443" || loc.port === "80") {
    return `${protocol}//${loc.hostname}/pi-terminal-mux`;
  }
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.addEventListener("error", () => {
        console.warn("[MuxProvider] WebSocket connection error (url:", url, ")");
      });
