...
import {
  TERMINAL_STATUSES as VI_TERMINAL_STATUSES,
  getProjectBaseDir,
  type VISession,
} from "@vi/core";
...
interface VIIdeaStore {
  ideas: VIIdeaRecord[];
}

const IDEA_COLUMNS: Array<{ id: VIIdeaStatus; title: string; description: string }> = [
  {
    id: "idea_bank",
    title: "Idea Bank",
...
const globalForVIIdeaExecution = globalThis as typeof globalThis & {
  _piIdeaSpawnLocks?: Map<string, Promise<VISession>>;
};

const DEFAULT_SERVER_EXECUTION_ROOT = "/srv/pi/workspaces";

function resolveIdeaExecutionRoot(): string {
  const explicit = process.env["VI_WORKSPACE_ROOT"]?.trim();
...
function deriveIdeaWorkspace(idea: Pick<VIIdeaRecord, "id" | "title" | "projectSlug" | "workspacePath">) {
...
function getIdeaSpawnLocks(): Map<string, Promise<VISession>> {
  if (!globalForVIIdeaExecution._piIdeaSpawnLocks) {
    globalForVIIdeaExecution._piIdeaSpawnLocks = new Map<string, Promise<VISession>>();
  }
  return globalForVIIdeaExecution._piIdeaSpawnLocks;
}

function sessionTimestamp(session: VISession): number {
...
function isIdeaStatus(value: string): value is VIIdeaStatus {
  return IDEA_COLUMNS.some((column) => column.id === value);
}

async function readIdeaStore(configPath: string, projectPath: string): Promise<VIIdeaStore> {
  try {
    const raw = await readFile(ideasPath(configPath, projectPath), "utf8");
    const parsed = JSON.parse(raw) as Partial<VIIdeaStore>;
    return {
      ideas: Array.isArray(parsed.ideas)
        ? parsed.ideas
            .filter(
              (idea): idea is VIIdeaRecord =>
                Boolean(
                  idea &&
...
function normalizeIdeaStore(store: VIIdeaStore): VIIdeaStore {
  const byColumn = new Map<VIIdeaStatus, VIIdeaRecord[]>(
...
async function writeIdeaStore(configPath: string, projectPath: string, store: VIIdeaStore): Promise<void> {
...
function serializeIdea(idea: VIIdeaRecord): VIIdeaCard {
  return {
    id: idea.id,
    title: idea.title,
    markdown: idea.markdown,
...
function buildIdeaPrompt(idea: Pick<VIIdeaRecord, "id" | "title" | "markdown">): string {
...
function findIdeaSessions(sessions: VISession[], projectId: string, ideaId: string): VISession[] {
  return sessions
    .filter((session) => {
      if (session.projectId !== projectId) return false;
      const prompt = session.metadata["userPrompt"] ?? "";
      return prompt.includes(`[VI_IDEA_ID:${ideaId}]`);
    })
    .sort((left, right) => sessionTimestamp(left) - sessionTimestamp(right));
}

function findActiveIdeaSession(sessions: VISession[], projectId: string, ideaId: string): VISession | null {
  return findIdeaSessions(sessions, projectId, ideaId).find(
    (session) => !TERMINAL_STATUSES.has(session.status),
  ) ?? null;
}

function findLatestIdeaSession(sessions: VISession[], projectId: string, ideaId: string): VISession | null {
...
function toBoard(projectId: string, store: VIIdeaStore): VIIdeaBoardData {
  const normalized = normalizeIdeaStore(store);
  const ideasByStatus = new Map<VIIdeaStatus, VIIdeaCard[]>(
    IDEA_COLUMNS.map((column) => [column.id, []]),
  );
  ...
  const columns: VIIdeaBoardColumnData[] = IDEA_COLUMNS.map((column) => ({
    id: column.id,
    title: column.title,
...
export async function resolveVIIdeaProjectId(projectId?: string | null): Promise<string> {
...
export function getVIIdeaExecutionRoot(): string {
  return resolveIdeaExecutionRoot();
}

export async function getVIIdeaBoard(projectId: string): Promise<VIIdeaBoardData> {
  const { config } = await getServices();
...
export async function createVIIdea(input: {
  projectId: string;
  title: string;
  markdown: string;
}): Promise<VIIdeaBoardData> {
  const { config } = await getServices();
  ...
  return getVIIdeaBoard(input.projectId);
}

export async function updateVIIdea(input: {
  projectId: string;
  ideaId: string;
  title: string;
  markdown: string;
...
export async function moveVIIdeas(input: {
  projectId: string;
  columns: Record<VIIdeaStatus, string[]>;
}): Promise<VIIdeaBoardData> {
  const { config } = await getServices();
  const resolved = await resolveProjectConfig(config, input.projectId);
  const store = normalizeIdeaStore(
