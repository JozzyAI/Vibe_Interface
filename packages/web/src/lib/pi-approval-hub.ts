...
import { getProjectBaseDir } from "@pi/core";
...
import type {
  ...
  PIApprovalAuditEntry,
  PIApprovalEventType,
  PIApprovalFleetItem,
  PIApprovalHubData,
  PIApprovalInboxEntry,
  PIApprovalPrimaryAction,
  PIApprovalPermissionMode,
  PIApprovalPolicySummary,
  PIApprovalRiskLevel,
  PIControlPlaneData,
} from "@/lib/types";
...
interface PIApprovalPolicyRecord {
  mode: PIApprovalPermissionMode;
  timeoutSeconds: number;
  updatedAt: string;
}

interface PIApprovalHubStore {
  projectPolicy: PIApprovalPolicyRecord;
  sessionPolicies: Record<string, PIApprovalPolicyRecord>;
  audit: PIApprovalAuditEntry[];
}
...
function defaultStore(): PIApprovalHubStore {
...
async function readStore(configPath: string, projectPath: string): Promise<PIApprovalHubStore> {
  try {
    const raw = await readFile(approvalHubStatePath(configPath, projectPath), "utf8");
    const parsed = JSON.parse(raw) as Partial<PIApprovalHubStore>;
    ...
    return {
      ...
      audit: Array.isArray(parsed.audit)
        ? parsed.audit.filter(
            (entry): entry is PIApprovalAuditEntry =>
              Boolean(
                entry &&
...
async function writeStore(configPath: string, projectPath: string, store: PIApprovalHubStore): Promise<void> {
...
function normalizePolicy(
  policy: PIApprovalPolicyRecord | undefined,
  fallback: PIApprovalPolicyRecord,
): PIApprovalPolicyRecord {
  return policy
    ? {
        mode: policy.mode,
...
export async function getPIApprovalPolicy(input: {
  projectId: string;
  sessionId?: string;
}): Promise<PIApprovalPolicySummary> {
...
function inboxRisk(kind: PIApprovalInboxEntry["kind"]): PIApprovalRiskLevel {
  switch (kind) {
    case "final_approval":
      return "high";
    case "plan_approval":
...
function inboxActionLabel(kind: PIApprovalInboxEntry["kind"]): string {
  switch (kind) {
    case "final_approval":
...
function requestEventType(kind: PIApprovalInboxEntry["kind"]): PIApprovalEventType {
  switch (kind) {
    case "final_approval":
...
function requestPrimaryAction(kind: PIApprovalInboxEntry["kind"]): PIApprovalPrimaryAction {
  switch (kind) {
    case "scope_clarification":
    case "example_request":
      return "reply";
...
function classifyNativeCommand(command: string | null | undefined): PIApprovalEventType {
...
function nativeActionLabel(eventType: PIApprovalEventType): string {
  switch (eventType) {
    case "network_access":
...
function nativeRisk(eventType: PIApprovalEventType): PIApprovalRiskLevel {
  switch (eventType) {
    case "git_push":
    case "delete_operation":
      return "high";
...
function fleetRisk(session: DashboardSession, pendingCount: number): PIApprovalRiskLevel {
...
export async function appendPIApprovalAuditEvent(
  configPath: string,
  projectPath: string,
  entry: Omit<PIApprovalAuditEntry, "id" | "createdAt"> & { createdAt?: string },
): Promise<PIApprovalAuditEntry> {
  const store = await readStore(configPath, projectPath);
  const auditEntry: PIApprovalAuditEntry = {
    id: `pia_${randomUUID()}`,
...
export async function setPIApprovalPolicy(input: {
  projectId: string;
  sessionId?: string;
  mode: PIApprovalPermissionMode;
  timeoutSeconds?: number;
  ...
  const nextPolicy: PIApprovalPolicyRecord = {
    mode: input.mode,
    timeoutSeconds,
    updatedAt,
  };
...
export async function getPIApprovalHubData(input: {
  projectId: string;
  sessions: DashboardSession[];
  controlPlane: PIControlPlaneData;
}): Promise<PIApprovalHubData> {
  const { config } = await getServices();
  const project = config.projects[input.projectId];
  if (!project) {
    throw new Error(`Unknown project: ${input.projectId}`);
  ...
  const projectPolicy: PIApprovalPolicySummary = {
    scopeType: "project",
  ...
  const fleet: PIApprovalFleetItem[] = input.sessions.map((session) => {
  ...
  const inbox: PIApprovalInboxEntry[] = visibleInboxItems.map((item) => {
  ...
  const nativeInbox: PIApprovalInboxEntry[] = input.sessions.flatMap((session) => {
  ...
  const policies: PIApprovalPolicySummary[] = [
    projectPolicy,
    ...fleet.map((item) => ({
